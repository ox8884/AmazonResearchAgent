...
export function CandidateBusinessInputs({ values, onChange, onRequestedApiPurposes }: { readonly values: BusinessFormValues; readonly onChange: (id: EditableTextField, value: string) => void; readonly onRequestedApiPurposes: (value: readonly string[]) => void }) {
  ...
  return <>
    ...
    <fieldset className="business-form__group" id="business-market"><legend>시장·체크</legend><div className="form-grid">
      <StatusField id="marketStatus" label="시장 검증" value={values.marketStatus} onChange={onChange} /><div className="field-stack"><label htmlFor="marketPeriodFrom">관측 시작 (UTC)</label><input id="marketPeriodFrom" type="datetime-local" value={values.marketPeriodFrom} onChange={(event) => onChange('marketPeriodFrom', event.currentTarget.value)} /></div><div className="field-stack"><label htmlFor="marketPeriodTo">관측 종료 (UTC)</label><input id="marketPeriodTo" type="datetime-local" value={values.marketPeriodTo} onChange={(event) => onChange('marketPeriodTo', event.currentTarget.value)} /></div>
      <div className="field-stack"><label htmlFor="marketSourceReference">시장 관측 출처 레퍼런스</label><input id="marketSourceReference" value={values.marketSourceReference} onChange={(event) => onChange('marketSourceReference', event.currentTarget.value)} /></div><div className="field-stack"><label htmlFor="marketSourceUrl">시장 관측 URL</label><input id="marketSourceUrl" type="url" placeholder="https://" value={values.marketSourceUrl} onChange={(event) => onChange('marketSourceUrl', event.currentTarget.value)} /></div><div className="field-stack"><label htmlFor="marketSourceBasis">시장 출처 성격</label><select id="marketSourceBasis" value={values.marketSourceBasis} onChange={(event) => onChange('marketSourceBasis', event.currentTarget.value)}><option value="observed">관측</option><option value="estimate">추정</option><option value="quote">견적</option></select></div><div className="field-stack"><label htmlFor="marketSourceRecordedAt">시장 관측 기록 시각 (UTC)</label><input id="marketSourceRecordedAt" type="datetime-local" value={values.marketSourceRecordedAt} onChange={(event) => onChange('marketSourceRecordedAt', event.currentTarget.value)} /></div>
      ...
      <p className="field-help">시장 관측과 공급처 견적은 별개입니다. 기록 시각을 직접 입력하세요.</p>
