...
- [ ] Write failing tests named `creates cross uid codex ipc with exact ownership and modes`, `denies codex access to grok ipc`, `publishes request only by exclusive tmp fsync rename`, `publishes result only by adapter-owned tmp fsync rename`, `rejects symlinks non-regular files wrong uid gid mode and oversize`, `starts adapter ExecStart only after request json`, `keeps unit active until worker reads result`, `cleans only after explicit stop and empty cgroup`, and `root gc removes only aged inactive crash orphans`. Also cover fixed adapter/unit selection, UUID-only instance, no direct client `spawn`, absolute binary/profile verification, empty environment, timeout/abort, `systemctl kill --kill-who=all`, and no unit start after verification failure. Add hostile policy fixtures for repo/worker.env/home/SSH/Hermes denial, auth-home/workspace-only writes, shell/tool denial, and resolver-only 53 plus endpoint-prefix-only 443. Run `pnpm --filter @ara/ai-router exec vitest run src/providers/subscription-process.test.ts src/providers/command.test.ts && pnpm --filter @ara/worker exec vitest run src/providers/systemd-subscription-sandbox.test.ts`; expected RED is missing the concrete cross-UID directory/file/lifecycle protocol, result-acknowledgment lifetime, and liveness-aware root GC.
...
- [ ] Adapter execution is the root-owned, digest-pinned `subscription-supervisor.mjs` under the fixed runtime in the unit `ExecStart`; neither path nor arguments are caller-controlled. It invokes only the fixed adapter client, captures/parses bounded output, publishes `result.json`, reaps the client, then remains alive without network/client work in a fixed `result-published` wait until the worker explicitly stops the unit. The retention timeout is greater than the transport's maximum execution/read timeout plus cleanup grace and exists only as a crash backstop. Result publication mirrors the request: open fixed `result.tmp` exclusively/no-follow at mode `0640`, require adapter UID plus IPC group, write one bounded schema-valid result including client exit evidence, fsync/close, require `result.json` absent, atomic same-directory rename, and directory fsync. The worker opens only fixed `result.json`, verifies regular file/adapter UID/group/mode/size before parse, and never traverses the adapter auth home. Optional diagnostics use only bounded `diagnostic.tmp` → `diagnostic.json`; `StandardOutput=null`, `StandardError=null`, and no journal read.
- [ ] Implement the named IPC helpers with directory file descriptors, numeric no-follow flags, `lstat`/`fstat`, and explicit maximums for request, result, diagnostics, individual frame, and aggregate parsed output. Partial tmp files are never consumed. Missing/malformed/oversize/wrong-owner output maps to the canonical fail-closed class. The unit is `Type=notify`; after publishing the result, the supervisor sends `STATUS=result-published`. A final result is authoritative only when fixed `systemctl show` properties report the supervisor active with exact `StatusText=result-published` and the provider-client exit evidence in the envelope agrees.
- [ ] Observe only fixed `systemctl show` properties `ActiveState`, `SubState`, `StatusText`, `ExecMainCode`, `ExecMainStatus`, and `Result` plus bounded final-file presence. Normal success order is publish result/status → worker verifies/reads/closes result → worker explicitly stops the fixed unit → wait inactive/failed and empty cgroup → root-owned `ExecStopPost=+.../manage-invocation.sh cleanup <adapter> <uuid>` validates/removes exactly that UUID directory without symlink traversal. On timeout/cancel/output violation call fixed-unit `systemctl kill --kill-who=all`, then the same inactive+cgroup-empty cleanup path. The worker/adapters never recursively delete cross-owned files. A root-owned GC timer scans only authoritative UUID directories older than the fixed retention window and removes one only after the matching unit is inactive/failed, its cgroup is empty, every path/object passes the same no-follow owner/group/mode checks, and a second liveness check still agrees; active/recent/ambiguous entries are retained and alerted. No generic tmpfiles age deletion is allowed.
...
- [ ] Run `node --test scripts/probe-subscription-provider.test.mjs && node --check scripts/probe-subscription-provider.mjs && node --check ops/subscription-providers/subscription-supervisor.mjs && bash -n ops/subscription-providers/install-systemd-sandbox.sh && bash -n ops/subscription-providers/install-auth-homes.sh && bash -n ops/subscription-providers/verify-runtime-profile.sh && git diff --check`; expected local GREEN validates exact cross-UID result lifecycle/root-GC fixtures, hostile profile fixtures, and Phase-A/fence/Phase-B runbook syntax. On Oracle additionally require `systemd-analyze verify ops/systemd/amazon-research-codex@.service ops/systemd/amazon-research-grok@.service ops/systemd/amazon-research-subscription-gc.service ops/systemd/amazon-research-subscription-gc.timer ops/systemd/amazon-research-worker.service`, hostile IPC/network/GC probes, and recorded writer/capability queries before acceptance. Commit only Task 14 files as `ops: prepare subscription provider acceptance`.
...
| Finding | Closure |
| --- | --- |
| I1 | Task 5 fixes exact cross-UID parent/invocation/file ownership and modes, exclusive tmp/fsync/atomic-final handoff, bounded schemas, fixed status observation, no-journal results, root post-cgroup `ExecStopPost` cleanup, and crash-orphan reconciliation; Task 14 installs and proves those same artifacts without the former 0700 contradiction. |
