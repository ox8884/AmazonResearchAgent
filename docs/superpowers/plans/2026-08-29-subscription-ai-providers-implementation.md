...
- Produces: `SubscriptionIpcProtocol`, the five canonical IPC helpers, `SystemdSubscriptionSandbox`, `SubscriptionProcessTransport.run(profile, invocation, signal)`, immutable unit/policy digests, bounded result/diagnostic envelopes, and unchanged test-only `CommandProvider` behavior.

- [ ] Write failing tests named `creates cross uid codex ipc with exact ownership and modes`, `denies codex access to grok ipc`, `publishes request only by exclusive tmp fsync rename`, `publishes result only by adapter-owned tmp fsync rename`, `rejects symlinks non-regular files wrong uid gid mode and oversize`, `starts adapter ExecStart only after request json`, `no READY times out Type notify startup`, `READY establishes active startup`, `STATUS without READY never establishes startup`, `READY then result status exposes distinct states`, `worker reads result before stop cleanup`, `stop terminates service then ExecStopPost cleans before inactive`, `worker never waits inactive before cleanup`, `timeout and cancellation stop kill clean then verify`, `failed start leaves no stale invocation directory`, and `root gc removes only aged inactive crash orphans`. Also cover fixed adapter/unit selection, UUID-only instance, no direct client `spawn`, absolute binary/profile verification, empty environment, `systemctl kill --kill-who=all`, and no unit start after verification failure. Add hostile policy fixtures for repo/worker.env/home/SSH/Hermes denial, auth-home/workspace-only writes, shell/tool denial, and resolver-only 53 plus endpoint-prefix-only 443. Run `pnpm --filter @ara/ai-router exec vitest run src/providers/subscription-process.test.ts src/providers/command.test.ts && pnpm --filter @ara/worker exec vitest run src/providers/systemd-subscription-sandbox.test.ts`; expected RED is that the current transport neither completes `Type=notify` startup through `READY=1` nor distinguishes running, result-published, and terminal states, and orders inactive observation before `ExecStopPost` cleanup.
...
- [ ] For a validated authoritative attempt UUID, `systemctl start --no-block amazon-research-<adapter>@<uuid>.service` runs the root-owned fixed `ExecStartPre=+.../manage-invocation.sh prepare-and-wait <adapter> <uuid>`. It creates exactly one invocation directory as `amazon-research:<adapter-ipc-group>` mode `2770`, rejects existing/symlinked/nonempty paths, and waits a bounded time for verified `request.json`; adapter `ExecStart` cannot begin before that final file exists. The polkit rule permits `amazon-research` only start/stop/kill and fixed-property observation for these two templates with canonical UUID instances; no transient property, executable, argv, path, UID, environment, or firewall value is caller-controlled.
- [ ] Worker request publication is exact: verify every parent/directory component, open directory-relative `request.tmp` with numeric `O_CREAT|O_EXCL|O_NOFOLLOW`, mode `0640`, expected owner `amazon-research` and inherited adapter IPC group, write one schema-valid bounded envelope, fsync file, close, verify `request.json` absent, atomically rename within the same directory, and fsync the directory. Adapter opens only fixed `request.json` with `O_NOFOLLOW`, verifies regular file/worker UID/group/mode/maximum bytes by `lstat` and `fstat`, then parses. Filenames never include model/provider/user data.
...
- [ ] Implement the named IPC helpers with directory file descriptors, numeric no-follow flags, `lstat`/`fstat`, and explicit maximums for request, result, diagnostics, individual frame, and aggregate parsed output. Partial tmp files are never consumed. Missing/malformed/oversize/wrong-owner output maps to the canonical fail-closed class. Each unit is `Type=notify` with `NotifyAccess=main`. The worker starts the fixed unit and, during the bounded start phase, uses allowlisted fixed `systemctl show` properties to require the active/running state established only by `READY=1`; `STATUS=result-published` updates status text only and never establishes readiness. During the separately bounded execution phase, exact `StatusText=result-published` is only a signal to validate and ingest fixed `result.json`; a final result is authoritative only when the active MAIN process has that exact status and provider-client exit evidence in the validated envelope agrees. Start timeout/failure before READY—including supervisor exit, request rejection, sandbox initialization failure, or `TimeoutStartSec`—never waits for result status and follows the existing canonical failure/consumption matrix without inventing a fallback class.
- [ ] Observe only fixed `systemctl show` properties `ActiveState`, `SubState`, `StatusText`, `ExecMainCode`, `ExecMainStatus`, and `Result` plus bounded final-file presence. Normal success is exact: `begin_ai_provider_attempt` commits → validated request publication → fixed-unit start → request/sandbox validation → `READY=1` → active/running → provider execution → `result.tmp` fsync/close and atomic `result.json` publication → `STATUS=result-published` → worker observes status → validates/reads/closes result → records the existing provider outcome → explicitly requests stop → systemd terminates service processes and all cgroup writers → root-owned `ExecStopPost=+.../manage-invocation.sh cleanup <adapter> <uuid>` validates the fixed UUID/path, refuses symlink/path escape or a live adapter, removes only that invocation directory, never touches auth homes or unrelated roots, and exits nonzero on unsafe inconsistency → stop transaction completes → unit reaches inactive/failed → worker verifies terminal state, empty cgroup, and absent invocation directory → existing logical finalization/fallback continues. `ExecStopPost` is part of the stop/failure transaction, never a worker action after inactive, and result publication does not trigger it. On provider timeout, output failure, `cancelled_by_caller`, `cancelled_by_job_lease_loss`, or `cancelled_by_shutdown` after start, preserve the canonical matrix classification, consumption, and fallback rules, request fixed-unit stop and, when required, `systemctl kill --kill-who=all`; systemd terminates the full cgroup, runs the same `ExecStopPost`, completes the stop transaction, then the worker verifies terminal/cgroup/directory postconditions before releasing lifecycle resources or the adapter semaphore. Cancellation before start needs no stop; cancellation after start request but before READY stops the starting unit through the same lifecycle. A pre-READY start failure is never treated as running, cannot wait indefinitely for result status, and must complete applicable failure/stop cleanup before the worker verifies no stale directory. A root-owned GC timer scans only authoritative UUID directories older than the fixed retention window and removes one only after the matching unit is inactive/failed, its cgroup is empty, every path/object passes the same no-follow owner/group/mode checks, and a second liveness check still agrees; active/recent/ambiguous entries are retained and alerted. No generic tmpfiles age deletion is allowed.
...
- [ ] Write failing fixture tests for wrong systemd version/cgroup mode/nftables support, UID/GID membership, parent/invocation/file owner/mode/path/digest, cross-adapter access, shared auth home, mutable unit/helper/GC/drop-in/ruleset, env/journal leakage, forbidden fs/exec/network access, atomic file publication, no-READY startup timeout, READY-established active state, STATUS-without-READY startup refusal, distinct READY-then-result status, result cleanup before worker read, inactive-wait-before-cleanup deadlock, cleanup-before-cgroup-stop, timeout/cancellation/start-failure cleanup order, active/recent/ambiguous GC refusal, aged inactive orphan removal, resolver/endpoint drift, changed bindings, wrong writer artifact/capability pairing, and incomplete writer fence. Run `node --test scripts/probe-subscription-provider.test.mjs`; expected RED is missing exact `Type=notify` readiness, result-status, stop/`ExecStopPost` lifecycle, installation/profile, and cutover verification tooling.
- [ ] Implement root-run installation for Ubuntu 24.04: systemd >=255, unified cgroup v2, nftables, polkit, non-login `ara-codex`/`ara-grok`, 0700 auth homes, exact `ara-codex-ipc`/`ara-grok-ipc` memberships, root/group 0750 parents, Task-5 templates with `Type=notify`/`NotifyAccess=main` and distinct bounded start/execution/stop deadlines, digest-pinned supervisor/lifecycle helper/GC timer/polkit/nftables policy, fixed drop-ins, configured resolvers, and endpoint prefix sets. Invocation directories are exactly 2770 and created only by the root lifecycle helper. Refuse generic tmpfiles deletion, unsupported capability, human/Hermes/shared homes, symlinks, repo-relative paths, mutable artifacts, cross-group membership, missing reject rules, or broad egress; print no secrets.
- [ ] Probe CLI invokes the same fixed unit/profile and hostile fixture adapter, emits bounded sanitized JSON, and exits nonzero for every missing category. On Oracle, without a live subscription-provider call, prove: unit properties are exactly `Type=notify` and `NotifyAccess=main`; no READY leaves startup incomplete and reaches bounded failure/cleanup; MAIN emits READY only after fixed identity/request/sandbox acceptance and the unit becomes active only then; STATUS without READY cannot establish startup; READY then atomic result publication changes exact `StatusText=result-published` while the unit remains active and `result.json` remains available; a worker-equivalent reader validates/ingests the result before explicit stop; explicit stop terminates process/cgroup writers, then runs `ExecStopPost`, then completes to terminal state; only after stop completion are inactive/failed, empty cgroup, and absent invocation directory asserted; timeout and cancellation use the same stop/kill → cleanup → terminal-verification order; and failed startup leaves no stale directory. Also verify `systemd-analyze verify/security`, artifact hashes, live nft semantic match, service UID/cgroup, parent/invocation/request/result ownership and mode, tmp→final atomic protocol, cross-adapter denial, root GC refusing active/recent/ambiguous entries and deleting only an aged inactive orphan, stdout/stderr null and no journal dependency, resolver-only 53, accepted-prefix-only 443, denial elsewhere, and auth persistence. Any failed denial, state transition, cleanup order, or drift blocks activation.
...
- [ ] Run `node --test scripts/probe-subscription-provider.test.mjs && node --check scripts/probe-subscription-provider.mjs && node --check ops/subscription-providers/subscription-supervisor.mjs && bash -n ops/subscription-providers/install-systemd-sandbox.sh && bash -n ops/subscription-providers/install-auth-homes.sh && bash -n ops/subscription-providers/verify-runtime-profile.sh && git diff --check`; expected local GREEN validates exact READY/result/stop state fixtures, cross-UID result retention through worker read, `ExecStopPost`-before-terminal cleanup, timeout/cancellation/start-failure cleanup, root-GC fixtures, hostile profile fixtures, and Phase-A/fence/Phase-B runbook syntax. On Oracle additionally require `systemd-analyze verify ops/systemd/amazon-research-codex@.service ops/systemd/amazon-research-grok@.service ops/systemd/amazon-research-subscription-gc.service ops/systemd/amazon-research-subscription-gc.timer ops/systemd/amazon-research-worker.service`, the fixture-adapter readiness/result/stop probes, hostile IPC/network/GC probes, and recorded writer/capability queries before acceptance. Commit only Task 14 files as `ops: prepare subscription provider acceptance`.
...
- After Task 4: local migrated database proves preflight, repeatable probe generation, activation CAS, dual lease epochs, atomic starts, immutable outcomes, exactly-once analysis and candidate-domain finalization, fenced no-winner transition, and immutable legacy writer capability.
- After Task 5: the exact cross-UID fixed-file IPC, no-journal result path, root lifecycle cleanup, concrete systemd templates, and policy artifacts exist before any adapter consumes them.
...
- After Task 14: exact Task-5 sandbox/IPC and Phase-A/fence/Phase-B runbook are installed/hostile-denial verified; tooling alone activates nothing.
...
| Finding | Closure |
| --- | --- |
| I1 | Task 5 preserves exact cross-UID parent/invocation/file ownership and modes, exclusive tmp/fsync/atomic-final handoff, bounded schemas, no-journal results, and liveness-aware root GC while completing `Type=notify`: MAIN emits `READY=1` exactly after request/sandbox acceptance, later `STATUS=result-published` remains distinct, result IPC survives worker ingestion, explicit stop terminates cgroup writers, `ExecStopPost` cleans during the stop transaction, and only then does the worker verify terminal/cgroup/directory state. Task 14 installs and proves those same normal, timeout, cancellation, and startup-failure transitions on Oracle with a fixture adapter, without the former 0700 contradiction. |
